...
import { 
  Plus, Trash2, Calculator, GraduationCap, RefreshCcw, Save, Info, Printer, 
  Moon, Sun, ChevronDown, ChevronUp, BookOpen, Target, LayoutGrid, Zap, ListTodo, X,
  Github, Linkedin, Mail, Globe
} from 'lucide-react';
import { motion, AnimatePresence } from 'framer-motion';
import { calculateGPA, calculateCGPA, getGradeValue, getGradeFromPercentage } from './utils/calculator';
...
const CourseRow = ({ course, onUpdate, onRemove }) => (
  <motion.div
...
const SemesterCard = ({ semester, onUpdate, onRemove, onAddCourse, onRemoveCourse, semesterIndex }) => {
  const [isOpen, setIsOpen] = useState(true);

  const sgpa = calculateGPA(
...
const QuickSemesterRow = ({ semester, onUpdate, onRemove, index }) => (
  <motion.div
...
const App = () => {
  const [activeTab, setActiveTab] = useState('detailed'); // 'detailed' or 'quick'
  const [semesters, setSemesters] = useState([{ id: Date.now(), courses: [{ id: 1, name: '', grade: '', credits: '', marks: '' }] }]);
  const [quickSemesters, setQuickSemesters] = useState([{ id: Date.now(), gpa: '', credits: '' }]);
  
  // Results State (Triggered)
  ...
  // Reset results and input data when switching tabs
  useEffect(() => {
    setResults({ cgpa: '0.00', credits: 0, semesterGPAs: [] });
    setSemesters([{ id: Date.now(), courses: [{ id: 1, name: '', grade: '', credits: '', marks: '' }] }]);
    setQuickSemesters([{ id: Date.now(), gpa: '', credits: '' }]);
  }, [activeTab]);
  ...
  const handleCalculate = () => {
    if (activeTab === 'detailed') {
      ...
      setResults({ cgpa, credits: currentCredits, semesterGPAs });
    } else {
      ...
      setResults({ cgpa, credits: totalCredits, semesterGPAs });
    }
  ...
  const addSemester = () => {
    setSemesters([...semesters, { id: Date.now(), courses: [{ id: Date.now() + 1, name: '', grade: '', credits: '', marks: '' }] }]);
  };

  const removeSemester = (id) => {
    if (semesters.length > 1) {
      setShowDeleteSemester(id);
    }
  };
  ...
  const confirmDeleteSemester = () => {
    if (showDeleteSemester) {
      if (activeTab === 'detailed') {
        setSemesters(semesters.filter((s) => s.id !== showDeleteSemester));
      } else {
        setQuickSemesters(quickSemesters.filter((s) => s.id !== showDeleteSemester));
      }
      setShowDeleteSemester(null);
    }
  };

  const updateSemester = (semesterIndex, courseIndex, field, value) => {
    const newSemesters = [...semesters];
    if (field === 'courses') {
      newSemesters[semesterIndex].courses = value;
    } else {
      let updatedCourse = { ...newSemesters[semesterIndex].courses[courseIndex], [field]: value };
      if (field === 'marks') {
        updatedCourse.grade = getGradeFromPercentage(value);
      }
      newSemesters[semesterIndex].courses[courseIndex] = updatedCourse;
    }
  ...
  const addCourseToSemester = (semesterIndex) => {
    const newSemesters = [...semesters];
    newSemesters[semesterIndex].courses.push({ id: Date.now(), name: '', grade: '', credits: '', marks: '' });
    setSemesters(newSemesters);
  };

  const removeCourseFromSemester = (semesterIndex, courseIndex) => {
    const newSemesters = [...semesters];
    if (newSemesters[semesterIndex].courses.length > 1) {
      newSemesters[semesterIndex].courses = newSemesters[semesterIndex].courses.filter((_, i) => i !== courseIndex);
      setSemesters(newSemesters);
    }
  };

  // Quick Mode Handlers
  const addQuickSemester = () => {
    setQuickSemesters([...quickSemesters, { id: Date.now(), gpa: '', credits: '' }]);
  };

  const removeQuickSemester = (id) => {
    if (quickSemesters.length > 1) {
      setShowDeleteSemester(id);
    }
  };

  const updateQuickSemester = (id, field, value) => {
    setQuickSemesters(quickSemesters.map(s => s.id === id ? { ...s, [field]: value } : s));
  };
    ...
    <div className="min-h-screen py-8 px-4 md:px-8 transition-colors duration-300">
      <div className="max-w-6xl mx-auto space-y-8">
        ...
        <header className="flex flex-col md:flex-row md:items-center justify-between gap-6 print:hidden">
          <div className="flex items-center gap-4">
            <div className="w-12 h-12 sm:w-14 sm:h-14 bg-brand-600 rounded-xl sm:rounded-2xl flex items-center justify-center shadow-2xl shadow-brand-500/40">
              <GraduationCap className="text-white w-6 h-6 sm:w-8 sm:h-8" />
            </div>
            <div>
              <h1 className="text-2xl sm:text-3xl md:text-4xl font-extrabold tracking-tight text-slate-900 dark:text-white">Superior GPA <span className="text-brand-600 dark:text-brand-500">Calculator</span></h1>
              <p className="text-xs sm:text-sm text-slate-600 dark:text-slate-400 font-semibold tracking-tight">Comprehensive academic performance tracking & analysis platform.</p>
            </div>
          </div>
          
          <div className="flex flex-wrap items-center gap-2 sm:gap-3">
             <div className="glass p-1 rounded-2xl flex gap-1 mr-2">
                <button 
                  onClick={() => setActiveTab('detailed')}
                  className={cn(
                    "px-3 sm:px-4 py-2 rounded-xl text-xs font-bold transition-all flex items-center gap-1.5 sm:gap-2 cursor-pointer",
                    activeTab === 'detailed' ? "bg-brand-600 text-white shadow-lg hover:scale-105" : "text-slate-500 hover:bg-slate-100 dark:hover:bg-slate-800 hover:scale-105"
                  )}
                >
                ...
                <button 
                  ...
                  className={cn(
                    "px-3 sm:px-4 py-2 rounded-xl text-xs font-bold transition-all flex items-center gap-1.5 sm:gap-2 cursor-pointer",
                    activeTab === 'quick' ? "bg-brand-600 text-white shadow-lg hover:scale-105" : "text-slate-500 hover:bg-slate-100 dark:hover:bg-slate-800 hover:scale-105"
                  )}
                >
            ...
            <button 
              ...
              className="p-2.5 sm:p-3 bg-brand-50 dark:bg-brand-900/20 text-brand-600 dark:text-brand-400 border border-brand-100 dark:border-brand-900/30 rounded-xl sm:rounded-2xl hover:shadow-lg transition-all cursor-pointer hover:scale-110"
            ...
            <button 
              ...
              className="p-2.5 sm:p-3 bg-white dark:bg-slate-800 text-slate-600 dark:text-slate-300 border border-slate-200 dark:border-slate-700 rounded-xl sm:rounded-2xl hover:shadow-lg transition-all cursor-pointer hover:scale-110"
            >
              {isDarkMode ? <Sun className="w-5 h-5 sm:w-6 sm:h-6" /> : <Moon className="w-5 h-5 sm:w-6 sm:h-6" />}
            </button>
            <button 
              onClick={() => window.print()}
              className="btn-secondary flex items-center gap-2 border-slate-200 dark:border-slate-700 px-3 sm:px-4 py-2.5 sm:py-3"
            >
            ...
            <button 
              onClick={resetAll}
              className="p-2.5 sm:p-3 bg-rose-50 dark:bg-rose-900/10 text-rose-600 border border-rose-100 dark:border-rose-900/30 rounded-xl sm:rounded-2xl hover:bg-rose-100 transition-all"
            >
          ...
          <div className={`${activeTab === 'detailed' ? 'md:col-span-3' : 'md:col-span-2'} glass rounded-2xl sm:rounded-[2.5rem] p-5 sm:p-6 md:p-8 flex flex-col md:flex-row items-center justify-around gap-6 sm:gap-8 relative overflow-hidden`}>
            ...
            <div className="text-center md:text-left transition-all">
              ...
              <div className="flex items-center justify-center md:justify-start gap-2 mt-3 text-slate-400">
                <div className="px-2.5 py-1 bg-brand-50 dark:bg-brand-900/30 text-brand-600 dark:text-brand-400 text-[10px] font-bold rounded-lg uppercase tracking-wider">
                  Superior Grade Match
                </div>
              </div>
            </div>
          </div>
                  ...
                  <AnimatePresence>
                    {quickSemesters.map((s, idx) => (
                      <QuickSemesterRow 
                        ...
                        onRemove={() => removeQuickSemester(s.id)}
                      />
                    ))}
                  </AnimatePresence>
               </div>
